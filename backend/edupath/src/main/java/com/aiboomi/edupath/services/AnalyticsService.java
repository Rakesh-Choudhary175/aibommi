...
public class AnalyticsService {
    ...
    public AnalyticsService(StudentRepository studentRepository, AcademicRecordRepository academicRepository, ActivityRepository activityRepository, CareerScoreRepository careerScoreRepository) {
    ...
    public Map<String, SubjectMetrics> computeSubjectMetrics(Student s) {
        ...
        Map<String, List<AcademicRecord>> bySubject = records.stream().collect(Collectors.groupingBy(AcademicRecord::getSubject));
        ...
        for (Map.Entry<String, List<AcademicRecord>> e : bySubject.entrySet()) {
            ...
            List<AcademicRecord> recs = e.getValue().stream().sorted(Comparator.comparing(AcademicRecord::getYear)).collect(Collectors.toList());
            ...
            double prevAvgPct = prev.isEmpty() ? latestPct : prev.stream().mapToDouble(r -> safePct(r.getMarks(), r.getMaxMarks())).average().orElse(latestPct);
    ...
    private double safePct(Double marks, Double maxMarks) {
        if (marks == null || maxMarks == null || maxMarks <= 0) return 0.0;
    ...
    public Map<String, Double> computeStrength(Student s) {
        return computeSubjectMetrics(s).entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, e -> e.getValue().subjectStrength));
    }
    ...
    public List<CareerScore> computeCareerScores() {
        ...
        // clear previous scores (optional: keep history - here we remove existing and replace)
    ...
    private CareerScore computeEngineering(Student s, Map<String, Double> strength, List<ExtracurricularActivity> activities) {
        // Locked weights
        ...
        if (hasRunOrSwim) confidence = ConfidenceLevel.HIGH;
        else if (present < weights.size()) confidence = ConfidenceLevel.LOW;

        return new CareerScore(s, Career.ENGINEERING, score, confidence);
    }

    private CareerScore computeDesign(Student s, Map<String, Double> strength, List<ExtracurricularActivity> activities) {
        ...
        ConfidenceLevel confidence = ConfidenceLevel.MEDIUM;
        if (art == -1 || math == null) confidence = ConfidenceLevel.LOW;
        // Extracurriculars doesn't change numeric score (per rules), may increase confidence if arts activities present
        boolean hasArtActivity = activities.stream().anyMatch(a -> nameContains(a, "art") || nameContains(a, "drawing") || nameContains(a, "design"));
        if (hasArtActivity) confidence = ConfidenceLevel.HIGH;
        return new CareerScore(s, Career.DESIGN, score, confidence);
    }

    private CareerScore computeMedicine(Student s, Map<String, Double> strength, List<ExtracurricularActivity> activities) {
        Double bio = strength.get("Biology");
        Double chem = strength.get("Chemistry");
        Double phys = strength.get("Physics");
        int present = 0; double numerator = 0.0;
        if (bio != null) { numerator += bio * 0.4; present++; }
        if (chem != null) { numerator += chem * 0.3; present++; }
        if (phys != null) { numerator += phys * 0.3; present++; }
        Double score = present == 0 ? null : clamp(numerator / (present==3?1:1), 0, 100); // weights sum to 1, missing handled by using available subset
        ConfidenceLevel confidence = ConfidenceLevel.MEDIUM;
        if (present < 3) confidence = ConfidenceLevel.LOW;
        // Extracurriculars only affect confidence
        boolean anyExtra = activities != null && !activities.isEmpty();
        if (anyExtra) confidence = ConfidenceLevel.HIGH;
        return new CareerScore(s, Career.MEDICINE, score, confidence);
    }

    private CareerScore computeSports(Student s, Map<String, Double> strength, List<ExtracurricularActivity> activities) {
        // PE academic
        ...
        Double runningScore = computeTimeBasedScore(activities.stream().filter(a -> nameContains(a, "run")).findFirst().orElse(null));
        Double swimmingScore = computeTimeBasedScore(activities.stream().filter(a -> nameContains(a, "swim")).findFirst().orElse(null));
        Double extracurricularScore = null;
        if (runningScore != null && swimmingScore != null) extracurricularScore = Math.max(runningScore, swimmingScore);
        else extracurricularScore = runningScore != null ? runningScore : swimmingScore;
    ...
    private CareerScore computeSinging(Student s, Map<String, Double> strength, List<ExtracurricularActivity> activities) {
        Double music = strength.get("Music");
        // find choir singing activity and its level
        ExtracurricularActivity choir = activities.stream().filter(a -> nameContains(a, "choir") || nameContains(a, "sing")).findFirst().orElse(null);
        Double choirScore = null;
        if (choir != null) {
            if (choir.getLevel() == CompetitionLevel.SCHOOL) choirScore = 70.0;
            else if (choir.getLevel() == CompetitionLevel.DISTRICT) choirScore = 85.0;
            else if (choir.getLevel() == CompetitionLevel.STATE) choirScore = 95.0;
        }
        Double finalScore;
        ConfidenceLevel confidence = ConfidenceLevel.MEDIUM;
        if (choirScore == null) {
    ...
    private boolean nameContains(ExtracurricularActivity a, String token) {
        if (a == null || a.getName() == null) return false;
    ...
    private Double computeTimeBasedScore(ExtracurricularActivity a) {
        if (a == null || a.getMetricValue() == null) return null;
        double time = a.getMetricValue();
        double base;
        if (time <= 25) base = 90;
        else if (time <= 27) base = 80;
        else if (time <= 30) base = 65;
        else base = 50;
        int bonus = 0;
        if (a.getLevel() == CompetitionLevel.DISTRICT) bonus = 10;
        else if (a.getLevel() == CompetitionLevel.STATE) bonus = 20;
    ...
    public List<CareerScoreDTO> analyzeAllStudentsAndReturnDTOs() {
        ...
        return all.stream().map(cs -> new CareerScoreDTO(cs.getStudent().getId(), cs.getCareer(), cs.getScore(), cs.getConfidence())).collect(Collectors.toList());
    }
}